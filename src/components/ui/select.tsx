...
>(({ className, children, size, ...props }, ref) => {
  ...
  const getSizeClasses = () => {
    ...
    // Default (md)
    return isTouchDevice ? "h-11 px-3 py-2 text-base" : "h-10 px-3 py-2 text-sm";
  };
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <SelectPrimitive.ScrollUpButton
      ...
      {...props}
    >
      <ChevronUp className={isTouchDevice ? "h-5 w-5" : "h-4 w-4"} />
    </SelectPrimitive.ScrollUpButton>
  );
});
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <SelectPrimitive.ScrollDownButton
      ...
      {...props}
    >
      <ChevronDown className={isTouchDevice ? "h-5 w-5" : "h-4 w-4"} />
    </SelectPrimitive.ScrollDownButton>
  );
});
    ...
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          ...
          // Responsive sizing
          deviceType === "mobile" ? "max-h-[50vh] min-w-[12rem]" : "max-h-96 min-w-[8rem]",
          // Touch optimization
          isTouchDevice && "touch-manipulation",
          position === "popper" &&
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <SelectPrimitive.Label
      ...
      className={cn(
        "font-semibold",
        isTouchDevice ? "py-2 pl-10 pr-3 text-base" : "py-1.5 pl-8 pr-2 text-sm",
        className
      )}
      {...props}
    />
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        ...
        isTouchDevice 
          ? "py-3 pl-10 pr-3 text-base min-h-[44px] touch-manipulation" 
    ...
    >
      <span className={cn(
        ...
        isTouchDevice ? "left-3 h-4 w-4" : "left-2 h-3.5 w-3.5"
      )}>
        <SelectPrimitive.ItemIndicator>
          <Check className={isTouchDevice ? "h-4 w-4" : "h-4 w-4"} />
        </SelectPrimitive.ItemIndicator>
      </span>
