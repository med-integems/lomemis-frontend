...
function Button({
  ...
  // Override size for touch devices to ensure minimum touch targets
  const responsiveSize = React.useMemo(() => {
    if (!isTouchDevice) return size;
    
    // Ensure minimum touch target size (44px) on touch devices
    if (size === "sm") return "default";
    if (size === "default") return "lg";
    return size;
  }, [size, isTouchDevice]);
  ...
  return (
    <Comp
      data-slot="button"
      className={cn(
        buttonVariants({ variant, size: responsiveSize }),
        // Touch optimization
